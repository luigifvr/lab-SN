...
{ 
  ...
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      Move();
      Measure();
      //PrintInstResults();     // Function used to print instantaneous results
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    stima_pres = rho * temp + (blk_av[iw]/blk_norm + ptail * (double)npart) / (3*vol); //Pressure
...
//g(r)
    ...
    for (int i=0; i<nbins; ++i) {
        ...
        err_gdir = Error(glob_av[i+igofr], glob_av2[i+igofr], iblk);
        
        Gofr <<  setw(wd) << gdir;
        Gave << setw(wd) <<  glob_av[i+igofr]/(double)iblk << setw(wd) << err_gdir;
    }
    Gofr << endl;
    Gave << endl;
...
void PrintInstResults(void){
    double epot, press;
    ofstream PrintEnergy, PrintPress;
    
    PrintEnergy.open("output.inst.epot.0", ios::app);
    PrintPress.open("output.inst.pres.0", ios::app);
    
    epot = walker[iv]/(double)npart + vtail;
    press = (walker[iw] + ptail*(double)npart)/(3*vol) + rho*temp;
    
    PrintEnergy << epot << endl;
    PrintPress << press << endl;
